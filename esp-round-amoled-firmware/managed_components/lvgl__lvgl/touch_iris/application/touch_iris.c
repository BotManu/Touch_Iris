...
{
    lv_touch_iris_widgets_components_init();
    ...
    if(disp_size == DISP_LARGE) {
        ...
        lv_obj_t * label = lv_touch_iris_widgets_title_create(tab_bar, "");
        ...
        lv_label_set_text_static(label, "Widgets touch_iris");
        lv_obj_add_flag(label, LV_OBJ_FLAG_IGNORE_LAYOUT);
        lv_obj_add_style(label, &style_text_muted, 0);
        lv_obj_align_to(label, logo, LV_ALIGN_OUT_RIGHT_BOTTOM, 10, 0);
    }

    lv_touch_iris_widgets_profile_create(t1);
    lv_touch_iris_widgets_analytics_create(t2);
    lv_touch_iris_widgets_shop_create(t3);

    color_changer_create(tv);
}


void lv_touch_iris_widgets_start_slideshow(void)
{
